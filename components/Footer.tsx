  ...
  return (
    <footer className="bg-ink text-bone mt-32">
      <div className="container-tv py-14">
        ...
        <div className="border-t border-bone/15 pt-6 mt-12 flex justify-between items-center flex-wrap gap-4">
          <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-bone/40">
            © Trainovate · Brand Hub v1.0
          </p>
          <p className="font-mono text-[10px] tracking-[0.14em] text-bone/40">
...
function FooterCol({ title, links }: { title: string; links: { href: string; label: string }[] }) {
  return (
    <div>
      <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-bone/40 mb-3">{title}</p>
      <ul className="space-y-2">
        {links.map((l) => (
          <li key={l.href}>
            <Link href={l.href} className="text-sm text-bone/75 hover:text-bone transition-colors">
              {l.label}
            </Link>
          </li>
        ))}
